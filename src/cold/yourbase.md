...
A Python or Ruby library would track which test was currently executing,
and the magic kernel code would trace system calls made during the life of the test.
If a file was read during a test,
that test was said to depend on that file.
And if no file depended on by a test had changed since last time the test was run,
...
these customers would rarely be nimble enough to swap entire CI platforms to
test drive a new product.
Hindsight is 20/20,
but it's clear now that it took a special type of company to agree to become a customer of ours.
