...
Strangely, the hyphen is the only dash that has an option here: the [nonbreaking hyphen](https://en.wikipedia.org/wiki/Hyphen#Nonbreaking_hyphens) or "hard hyphen" is a hyphen glyph that sends a signal to the word wrapper that a wrap must not happen after it.
